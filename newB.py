...
def BFS_for_robot(index: int):
    if len(frontier) == 0 and len(explored) != 0:
        ...
        print("bfs robot is ", nd.robot_loc, nd.depth, nd.butter_loc, nd.last_move)
        BFS_for_robot(index)


def BFS_for_goal(index: int):
    if len(frontier_for_goal) == 0 and len(explored_for_goal) != 0:
        ...
        # print("z" , frontier[0].robot_loc)
        # print("___________________________")
        print("bfs goal is", nd.goal_loc[index], nd.depth, nd.last_move)
        BFS_for_goal(index)


def check_goal_bi(index: int):
    for i in range(len(frontier_for_goal)):
        if frontier[(len(frontier) - 1)].butter_loc == frontier_for_goal[i].goal_loc[index]:
            location = frontier[(len(frontier) - 1)].butter_loc
            up = (location[0] - 1, location[1])
            down = (location[0] + 1, location[1])
            left = (location[0], location[1] - 1)
            right = (location[0], location[1] + 1)
            if frontier[(len(frontier) - 1)].robot_loc == up or frontier[(len(frontier) - 1)].robot_loc == down or \
                    frontier[(len(frontier) - 1)].robot_loc == left or frontier[(len(frontier) - 1)].robot_loc == right:
                return True
    return False
...
def bid(index: int):
    for i in range(len(frontier_for_goal)):
        for j in range(len(frontier)):
            if (frontier_for_goal[i].goal_loc[index] in frontier[j].butter_loc) and (
                    frontier[j].parent.butter_loc[index] == frontier[j].robot_loc):
                location = frontier[j].butter_loc[index]
                up = (location[0] - 1, location[1])
                down = (location[0] + 1, location[1])
                left = (location[0], location[1] - 1)
                right = (location[0], location[1] + 1)
                if frontier[j].robot_loc == up or frontier[j].robot_loc == down or frontier[j].robot_loc == left or \
                        frontier[j].robot_loc == right:
                    print("\n\n|||||We won robot, butter, goal\n", frontier[j].robot_loc, frontier[j].butter_loc[index],
                          frontier_for_goal[i].goal_loc[index])
                    return

                    # it's for BFS_for_robot
    if len(frontier) == 0 and len(explored) != 0:
        return
    else:
        tmp_check = True
        ...
        for i in range(len(frontier_for_goal)):
            for j in range(len(frontier_for_goal)):
                ...
                if tmp_check and frontier_for_goal[i].goal_loc[index] == frontier_for_goal[j].goal_loc[index] and \
                    ...
                    tmp_check = False
                    break
        goal_nd = frontier_for_goal.pop(0)
        explored_for_goal.append(goal_nd)
        goal_ch = generate_goal_children(goal_nd, index)

    for i in range(len(goal_ch)):
        condition = True
        for j in range(len(explored_for_goal)):
            if goal_ch[i].goal_loc[index] == explored_for_goal[j].goal_loc[index] and set(
                    goal_ch[i].goal_loc[index]) == set(explored_for_goal[j].goal_loc[index]):
                if goal_ch[i].cost < explored_for_goal[j].cost:
                    explored_for_goal.pop(j)
                else:
                    condition = False
                break
        if condition:
            frontier_for_goal.append(goal_ch[i])

    # bfs for robot
    print("bid robot is ", nd.robot_loc, nd.depth, nd.butter_loc, nd.last_move)
    print("bid goal is", goal_nd.goal_loc[index], goal_nd.depth, goal_nd.last_move)
    print("_____________")
    bid(index)
...
start = Node(robot_location, butter_location, None, None, int(environment[robot_location[0]][robot_location[1]][0]))
frontier.append(start)
purpose = NodeForGoal(goal_location, None, None, int(environment[goal_location[0][0]][goal_location[0][1]][0]))
frontier_for_goal.append(purpose)
bid(0)
frontier.clear()
explored.clear()
